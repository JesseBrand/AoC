...
public class D06 {
	...
	public static void main(String[] args) throws IOException {
		...
		final List<Race> races = parseRaces(lines);
...
//		a = 1; b = -time; c = distance
		...
		System.out.println(String.format("Quadratic Formula: %s -> %d-%d -> %d",
				Arrays.toString(formulaResult),
				(int) Math.floor(formulaResult[0]), (int) Math.ceil(formulaResult[1]), result));
		return result;
	}

	private static List<Race> parseRaces(final List<String> lines) {
