...
import nl.jessebrand.aoc.aoc2022.D16.Move;
...
public class D16b2 {
	...
	private static Move findOptimalPath(List<Valve> valves, Map<String, Map<String, Integer>> paths) {
		...
		possibleSolutions = possibleSolutions.stream().filter(m -> m.elapsedMinutes() >= MAX_MINUTES - longest).toList();
		System.out.println("Continuing to 2nd part with " + possibleSolutions.size() + " possible solutions");
		...
		int i = 0;
		int totalSolutions = possibleSolutions.size();
		for (final Move endMove : possibleSolutions) {
			i++;
			if (i % 1000 == 1) {
				System.out.println("Evaluating option " + i + "/" + totalSolutions + "; highest so far = " + highestFlow);
			}
			possibleSolutions = new ArrayList<>();
	
			nextAttempts = new ArrayList<>();
			nextAttempts.add(new Move("AA", 0, endMove.totalFlowRate(), new ArrayList<>(endMove.closedValves()), new ArrayList<>(endMove.openedValves()), null));
...
//				System.out.println("Evaluating " + nextCombiAttempts.size() + " possibilities (" + lowestTime + " to " + highestTime + " min)");
				List<Move> nextNextAttempts = new ArrayList<>();
				for (final Move nextAttempt : nextAttempts) {
					possibleSolutions.add(nextAttempt);
					for (final Valve target : nextAttempt.closedValves()) {
